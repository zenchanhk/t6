...
class FTController:
    ...
    async def _open_trade(self, market):
        ...
        # self._hktrade.connectEvent += Messenger.MESSENGER.on_status
        # self._hktrade.errorEvent += Messenger.MESSENGER.on_error
        return '{"ret_val": 0}'
    ...
    def place_order(self, order_list, limit, **kwargs):
        future = asyncio.run_coroutine_threadsafe(
            self.place_order_async(order_list, limit, **kwargs), self.loop)
        return future.result()
    ...
    @asyncio.coroutine
    async def place_order_async(self, order_list, limit, **kwargs):
        list = json.loads(order_list)
        for o in list:            
            getattr(self, TradePlatMap[o['Market']]['var']).place_order(price=o['Price'], 
